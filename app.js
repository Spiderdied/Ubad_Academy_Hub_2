...
const LAYERS={};
/* ── WELCOME INTRO — سينمائي متعدد المراحل (محتوى Site الأصلي حرفيًا) ── */
LAYERS.welcome={
  title:()=>t('nav.hub'),
  render(){
    const sec=document.createElement('section'); sec.className='layer wv';
    /* المحتوى حرفيًا من الصفحة الأصلية */
    const WV={
      en:{skip:'Skip',choose:'Choose your language',
        eyebrow:'SYSTEM · ACADEMIC YEAR INITIALIZATION',h1:'Welcome',
        sub:'To the beginning of a new chapter.',
        ready1:'Loading complete.',ready2:'All systems are ready.',
        w1:'Welcome to a new academic year.',w2:'A year whose story has not been written yet.',
        w3:'Faces we already know,',w4:'faces we are yet to meet,',
        w5:'and days that will one day become memories.',
        track:'English Basic Education',level:'Second Year',yrs:'2026 — 2027',
        f1:'Last year became a memory.',f2:'But this year...',
        f3:'you are the ones who will write its story.',begin:"Let's Begin ✦",
        c1:'Welcome, everyone. ❤️',c2:'May this year be better than the last.',
        seq:[{t:'Initializing system...',cls:''},
          {t:'Loading the new academic year... ✓',cls:'tag-ok',pct:8},
          {t:'Preparing memory... ✓',cls:'tag-ok',pct:16},
          {t:'Loading ambitions... ✓',cls:'tag-ok',pct:24},
          {t:'Searching for motivation...',cls:''},
          {t:'FAILED.',cls:'tag-fail'},
          {t:'Retrying...',cls:''},
          {t:'Searching for motivation...',cls:''},
          {t:'Found: 12%.',cls:'tag-warn',pct:34},
          {t:'Loading brain for studying...',cls:'',pct:52},
          {t:'WARNING: limited brain storage available.',cls:'tag-warn'},
          {t:'Deleting memories from last year...',cls:''},
          {t:'FAILED. Those memories are stubborn.',cls:'tag-fail'},
          {t:'Installing new knowledge...',cls:'',pct:71},
          {t:'ERROR 404: Focus not found.',cls:'tag-fail'},
          {t:'Retrying...',cls:''},
          {t:'Focus found! It appears to have been sleeping.',cls:'tag-ok',pct:88},
          {t:'Calibrating patience for 8 a.m. lectures...',cls:'',pct:94},
          {t:'Almost there.',cls:'tag-emph',pct:100}]},
      ar:{skip:'تخطي',choose:'اختر لغتك',
        eyebrow:'النظام · تهيئة العام الدراسي',h1:'مرحبًا بك',
        sub:'في بداية فصلٍ جديد.',
        ready1:'اكتمل التحميل.',ready2:'جميع الأنظمة جاهزة.',
        w1:'أهلًا بكم في عامٍ جديد.',w2:'عامٌ لم تبدأ حكايته بعد.',
        w3:'وجوهٌ نعرفها,',w4:'ووجوهٌ سنلتقي بها,',
        w5:'وأيامٌ ستصبح يومًا ما ذكريات.',
        track:'أساسي إنجليزي',level:'الفرقة الثانية',yrs:'2026 — 2027',
        f1:'العام الماضي أصبح ذكرى.',f2:'أما هذا العام...',
        f3:'فأنتم من سيكتب حكايته.',begin:'لنبدأ الحكاية ✦',
        c1:'مرحبًا بكم جميعًا ❤️',c2:'نتمنى أن يكون هذا العام أفضل من كل عام مضى.',
        seq:[{t:'جاري تشغيل النظام...',cls:''},
          {t:'تحميل العام الدراسي الجديد... ✓',cls:'tag-ok',pct:8},
          {t:'تجهيز الذاكرة... ✓',cls:'tag-ok',pct:16},
          {t:'تحميل الطموحات... ✓',cls:'tag-ok',pct:24},
          {t:'البحث عن الحماس...',cls:''},
          {t:'فشل.',cls:'tag-fail'},
          {t:'إعادة المحاولة...',cls:''},
          {t:'البحث عن الحماس...',cls:''},
          {t:'تم العثور على 12%.',cls:'tag-warn',pct:34},
          {t:'تحميل العقل للدراسة...',cls:'',pct:52},
          {t:'تحذير: مساحة التخزين العقلي محدودة.',cls:'tag-warn'},
          {t:'حذف بعض ذكريات العام الماضي...',cls:''},
          {t:'فشل. الذكريات عنيدة.',cls:'tag-fail'},
          {t:'تثبيت المعرفة الجديدة...',cls:'',pct:71},
          {t:'خطأ 404: التركيز غير موجود.',cls:'tag-fail'},
          {t:'إعادة المحاولة...',cls:''},
          {t:'تم العثور عليه! يبدو أنه كان نائمًا.',cls:'tag-ok',pct:88},
          {t:'ضبط الصبر لمحاضرات الثامنة صباحًا...',cls:'',pct:94},
          {t:'أوشكنا على الانتهاء.',cls:'tag-emph',pct:100}],
        bonus:{surprise:'هو....... حضرتك مستني حاجة تاني؟ 😶',
          q:'طب ما دام استنيت ممكن سؤال؟\nليه اخترت عربي ومخترتش إنجليزي؟',
          opts:['١. علشان دي لغتي وبعتز بيها ❤️','٢. وانت مالك؟ 😂','٣. الإنجليزي بتاعي ضعيف 😐'],
          ans:{
            1:'برافو عليك 👏\n\nمش معنى إننا بنتعلم إنجليزي إننا نفضلها على لغتنا العربية، لغة القرآن، وإنما بنتعلمها علشان نوسع آفاقنا.\n\nوكما جاء في القول المأثور (وليس حديثًا نبويًا):\n\n«من تعلّم لغة قوم أمن مكرهم»',
            2:'خلاص يا عم متزقش😒\nمكنش سؤال يعني.',
            3:'ولا يهمك يا بطل ✌️\n\nحاول ومتستسلمش،\nوبإذن الله هتوصل ❤️'}}}
    };

    let lang=(state.settings.lang==='ar')?'ar':'en';
    let dead=false, entered=false, surTimer=0;
    const SPD=RM.matches?0.3:1;
    const timers=[]; const T=(fn,ms)=>{ const id=setTimeout(()=>{ if(!dead) fn(); },ms*SPD); timers.push(id); return id; };

    /* ── أصوات مركّبة محلية (تحترم إعداد الصوت) ── */
    let AC=null;
    ...
    const wt=(f,o={})=>{ if(!state.settings.sound) return; try{ const c=ac(); if(!c) return;
      const t0=c.currentTime+(o.at||0), d=o.dur||.2;
      const osc=c.createOscillator(); osc.type=o.type||'sine'; osc.frequency.setValueAtTime(f,t0);
      if(o.slide) osc.frequency.exponentialRampToValueAtTime(Math.max(1,o.slide),t0+d);
      const g=c.createGain(); g.gain.setValueAtTime(.0001,t0);
      g.gain.exponentialRampToValueAtTime(o.g||.04,t0+.02);
      g.gain.exponentialRampToValueAtTime(.0001,t0+d);
      osc.connect(g); g.connect(c.destination); osc.start(t0); osc.stop(t0+d+.05); }catch(e){} };
    const wnoise=(o={})=>{ if(!state.settings.sound) return; try{ const c=ac(); if(!c) return;
      const t0=c.currentTime, d=o.dur||.4;
      const buf=c.createBuffer(1,Math.max(1,(c.sampleRate*d)|0),c.sampleRate);
      const ch=buf.getChannelData(0);
      for(let i=0;i<ch.length;i++) ch[i]=(Math.random()*2-1)*(1-i/ch.length);
      const s=c.createBufferSource(); s.buffer=buf;
      const f=c.createBiquadFilter(); f.type='bandpass'; f.frequency.value=o.freq||1800;
      const g=c.createGain(); g.gain.setValueAtTime(o.g||.05,t0);
      g.gain.exponentialRampToValueAtTime(.0001,t0+d);
      s.connect(f); f.connect(g); g.connect(c.destination); s.start(t0); s.stop(t0+d+.02); }catch(e){} };
    const snd={
      sweep(){ wt(220,{dur:.5,slide:440,g:.025}); },
      ok(){ wt(660,{dur:.12}); wt(990,{dur:.18,at:.08,g:.03}); },
      warn(){ wt(340,{dur:.18,type:'triangle'}); wt(300,{dur:.18,at:.14,type:'triangle',g:.025}); },
      fail(){ wt(220,{dur:.22,slide:140}); },
      complete(){ wt(440,{dur:.15}); wt(660,{dur:.2,at:.1}); wt(880,{dur:.35,at:.2,g:.035}); },
      reveal(){ wt(520,{dur:.4,g:.025}); wt(780,{dur:.5,at:.12,g:.02}); },
      hero(){ wt(392,{dur:.6,g:.022}); wt(587,{dur:.8,at:.2,g:.02}); wt(784,{dur:1,at:.4,g:.016}); },
      whoosh(){ wnoise({dur:.4}); wt(120,{dur:.4,slide:500}); },
      surprise(){ wt(300,{dur:.1}); wt(500,{dur:.1,at:.09}); wt(400,{dur:.15,at:.18,g:.025}); },
      warm(){ wt(523,{dur:.25}); wt(659,{dur:.3,at:.1,g:.025}); wt(784,{dur:.4,at:.2,g:.02}); },
      humor(){ wt(400,{dur:.3,slide:250,type:'triangle'}); },
      encourage(){ wt(440,{dur:.2}); wt(660,{dur:.3,at:.1,g:.028}); }
    };

    /* ── الهيكل ── */
    sec.innerHTML=`
      <div class="wv-flash"></div>
      <button class="wv-skip" id="wv-skip"></button>
      <div class="wv-body" id="wv-body"></div>`;
    const body=$('#wv-body',sec);

    const NODES=[[50,20],[35,30],[65,30],[25,45],[50,42],[75,45],[20,62],[38,60],[62,60],[80,62],[32,78],[50,82],[68,78]];
    const EDGES=[[0,1],[0,2],[1,3],[1,4],[2,4],[2,5],[3,6],[4,6],[4,7],[4,8],[5,8],[5,9],[6,10],[7,10],[7,11],[8,11],[8,12],[9,12],[3,4],[4,5]];

    function build(){
      const c=WV[lang], b=(lang==='ar');
      let brain='<circle class="wv-core" cx="50" cy="50" r="30"/>';
      EDGES.forEach((e,i)=>{ brain+=`<line class="wv-edge" id="we${i}" x1="${NODES[e[0]][0]}" y1="${NODES[e[0]][1]}" x2="${NODES[e[1]][0]}" y2="${NODES[e[1]][1]}"/>`; });
      NODES.forEach((n,i)=>{ brain+=`<circle class="wv-node" id="wn${i}" cx="${n[0]}" cy="${n[1]}" r="2.1"/>`; });
      const opt=txt=>`<button class="wv-lang ar">${esc(txt)}</button>`;
      body.innerHTML=`
      <section class="wv-stage on" data-s="lang">
        <div class="wv-eyebrow">${esc(c.eyebrow)}</div>
        <h1 class="wv-h1 wv-serif">${esc(c.h1)}</h1>
        <p class="wv-sub wv-serif">${esc(c.sub)}</p>
        <p class="wv-choose">${esc(c.choose)}</p>
        <div class="wv-langs">
          <button class="wv-lang ar" data-l="ar">العربية</button>
          <button class="wv-lang" data-l="en">English</button>
        </div>
      </section>
      <section class="wv-stage" data-s="load">
        <div class="wv-brain"><svg viewBox="0 0 100 100" aria-hidden="true">${brain}</svg></div>
        <div class="wv-console" id="wv-con"></div>
        <div class="wv-bar"><div class="wv-fill" id="wv-fill"></div></div>
        <div class="wv-pct" id="wv-pct">0%</div>
      </section>
      <section class="wv-stage" data-s="ready">
        <h2 class="wv-soft wv-serif" style="font-size:clamp(1.6rem,5vw,2.7rem)">${esc(c.ready1)}</h2>
        <h2 class="wv-soft wv-l2 wv-serif" style="font-size:clamp(1.6rem,5vw,2.7rem)">${esc(c.ready2)}</h2>
      </section>
      <section class="wv-stage" data-s="poem">
        <div class="wv-wline wv-serif">${esc(c.w1)}</div>
        <div class="wv-wline wv-serif">${esc(c.w2)}</div>
        <div style="height:1.2rem"></div>
        <div class="wv-wline wv-small">${esc(c.w3)}</div>
        <div class="wv-wline wv-small">${esc(c.w4)}</div>
        <div class="wv-wline wv-small">${esc(c.w5)}</div>
      </section>
      <section class="wv-stage" data-s="year">
        <div class="wv-year">
          <div class="wv-track wv-serif">${esc(c.track)}</div>
          <div class="wv-level">${esc(c.level)}</div>
          <div class="wv-yrs">${esc(c.yrs)}</div>
        </div>
      </section>
      <section class="wv-stage" data-s="final">
        <div class="wv-fline wv-serif">${esc(c.f1)}</div>
        <div class="wv-fline wv-serif">${esc(c.f2)}</div>
        <div class="wv-fline wv-hero wv-serif">${esc(c.f3)}</div>
        <button class="wv-begin wv-serif" id="wv-begin">${esc(c.begin)}</button>
      </section>
      <section class="wv-stage" data-s="closing">
        <div class="wv-cline wv-serif" id="wv-c1">${esc(c.c1)}</div>
        <div class="wv-csub" id="wv-c2">${esc(c.c2)}</div>
        <button class="wv-ghost" id="wv-ghost">✦ ${esc(t('welcome.enter'))}</button>
      </section>
      <section class="wv-stage" data-s="bonus">
        <div class="wv-cline wv-serif wv-pre" id="wv-sur"></div>
        <div class="wv-cline wv-serif wv-pre" id="wv-bq" style="margin-top:1.4rem;display:none"></div>
        <div class="wv-langs" id="wv-bopts" style="display:none">
          ${WV.ar.bonus.opts.map((o,i)=>`<button class="wv-lang ar" data-b="${i+1}">${opt?esc(o):''}</button>`).join('')}
        </div>
        <div class="wv-cline wv-pre" id="wv-bans" style="display:none;font-size:clamp(1rem,3vw,1.35rem)"></div>
        <button class="wv-enter btn btn-primary" id="wv-benter">✦ ${esc(t('welcome.enter'))}</button>
      </section>`;
      $('#wv-skip',sec).textContent=c.skip;
      wire();
    }

    function show(id){ $$('.wv-stage',body).forEach(s=>s.classList.toggle('on',s.dataset.s===id)); }

    function wire(){
      $$('.wv-stage[data-s="lang"] .wv-lang',body).forEach(b=>b.addEventListener('click',()=>{
        try{ ac(); }catch(e){}
        wt(1200,{dur:.06}); wt(1800,{dur:.04,at:.01,g:.02});
        lang=b.dataset.l; state.settings.lang=lang; saveData(); applyLang();
        build(); /* إعادة بناء بلغة/اتجاه الجديد — ثم متابعة */
        T(()=>{ show('load'); startLoad(); },800);
      }));
      const bg=$('#wv-begin',body); if(bg) bg.addEventListener('click',beginFinale);
      const gh=$('#wv-ghost',body); if(gh) gh.addEventListener('click',goHub);
      const be=$('#wv-benter',body); if(be) be.addEventListener('click',goHub);
      const sur=$('#wv-sur',body); if(sur) sur.addEventListener('click',()=>{ if(surTimer){ clearTimeout(surTimer); surTimer=0; bonusQ(); } });
      const bo=$('#wv-bopts',body); if(bo) bo.addEventListener('click',e=>{
        const b=e.target.closest('[data-b]'); if(b) bonusAns(+b.dataset.b); });
    }

    /* ── التحميل ── */
    function hold(it){ const len=it.t.length; let b;
      if(it.cls==='tag-emph') b=2500; else if(len<=12) b=900; else if(len<=26) b=1000;
      else if(len<=40) b=2000; else b=2500;
      if(it.cls==='tag-warn') b+=400; if(it.cls==='tag-ok') b+=300;
      if(it.pct!==undefined) b+=500; return b; }
    function line(con,it){ while(con.children.length>=2) con.removeChild(con.firstChild);
      const d=document.createElement('div');
      d.className='wv-line'+(it.cls?{
        'tag-ok':' wv-ok','tag-warn':' wv-warn','tag-fail':' wv-fail',
        'tag-emph':' wv-emph wv-serif'}[it.cls]:'');
      d.textContent=it.t; con.appendChild(d);
      requestAnimationFrame(()=>d.classList.add('show'));
      if(it.cls==='tag-ok') snd.ok(); else if(it.cls==='tag-warn') snd.warn();
      else if(it.cls==='tag-fail') snd.fail(); }
    function prog(p){ const f=$('#wv-fill',body), pc=$('#wv-pct',body);
      if(f) f.style.width=p+'%'; if(pc) pc.textContent=p+'%';
      const nn=Math.round(p/100*NODES.length), ne=Math.round(p/100*EDGES.length);
      for(let i=0;i<NODES.length;i++){ const el=document.getElementById('wn'+i); if(el) el.classList.toggle('lit',i<nn); }
      for(let i=0;i<EDGES.length;i++){ const el=document.getElementById('we'+i); if(el) el.classList.toggle('lit',i<ne); }
      if(p===100) snd.complete(); }
    function startLoad(){ snd.sweep(); const con=$('#wv-con',body); con.innerHTML='';
      const seq=WV[lang].seq; let acc=500;
      seq.forEach((it,i)=>{ T(()=>{ line(con,it); if(it.pct!==undefined) prog(it.pct);
        if(i===seq.length-1) T(finishLoad,2600); },acc); acc+=hold(it); }); }

    /* ── التسلسل ── */
    function finishLoad(){ show('ready');
      T(()=>{ show('poem');
        $$('.wv-stage[data-s="poem"] .wv-wline',body).forEach((el,i)=>
          T(()=>el.classList.add('show'),400+i*1300));
        T(()=>{ show('year'); snd.reveal(); },400+5*1300+2300);
        T(()=>{ show('final');
          const fl=$$('.wv-stage[data-s="final"] .wv-fline',body);
          [[0,400],[1,2400],[2,4400]].forEach(([i,at])=>T(()=>{
            fl[i].classList.add('show'); if(fl[i].classList.contains('wv-hero')) snd.hero(); },at));
          T(()=>{ const b=$('#wv-begin',body); if(b) b.classList.add('show'); },6200);
        },4200);
      },3200); }

    function beginFinale(){ if(entered) return; entered=true;
      const fl=$('.wv-flash',sec); fl.classList.remove('burst'); void fl.offsetWidth;
      fl.classList.add('burst'); snd.whoosh(); FX.confetti();
      T(()=>{ show('closing'); entered=false;
        T(()=>{ const c1=$('#wv-c1',body); if(c1) c1.classList.add('show'); },200);
        T(()=>{ const c2=$('#wv-c2',body); if(c2) c2.classList.add('show'); },1100);
        T(()=>{ const g=$('#wv-ghost',body); if(g) g.classList.add('show'); },4000);
        if(lang==='ar') T(()=>{ if(!dead&&!entered) bonusSurprise(); },20000);
      },750); }

    /* ── البونص العربي الخفي ── */
    function bonusSurprise(){ show('bonus');
      const sur=$('#wv-sur',body); sur.textContent=WV.ar.bonus.surprise;
      snd.surprise(); requestAnimationFrame(()=>sur.classList.add('show'));
      surTimer=T(bonusQ,30000); }
    function bonusQ(){ if(dead) return; surTimer=0;
      const sur=$('#wv-sur',body), q=$('#wv-bq',body), bo=$('#wv-bopts',body);
      if(!sur||!q) return;
      sur.classList.remove('show');
      T(()=>{ sur.style.display='none';
        q.textContent=WV.ar.bonus.q; q.style.display='block';
        snd.reveal(); requestAnimationFrame(()=>q.classList.add('show'));
        T(()=>{ bo.style.display='flex'; requestAnimationFrame(()=>bo.classList.add('show')); },700);
      },300); }
    function bonusAns(n){ const q=$('#wv-bq',body), bo=$('#wv-bopts',body), an=$('#wv-bans',body);
      if(q){ q.classList.remove('show'); q.style.display='none'; }
      if(bo){ bo.classList.remove('show'); bo.style.display='none'; }
      [snd.warm,snd.humor,snd.encourage][n-1]&&[snd.warm,snd.humor,snd.encourage][n-1]();
      T(()=>{ an.textContent=WV.ar.bonus.ans[n]||''; an.style.display='block';
        requestAnimationFrame(()=>an.classList.add('show'));
        T(()=>{ const be=$('#wv-benter',body); if(be) be.classList.add('show'); },1500);
      },250); }

    /* ── الدخول للأكاديمية ── */
    function goHub(){ if(dead) return; dead=true;
      timers.forEach(clearTimeout);
      try{ localStorage.setItem('ubad.seen.v1','1'); }catch(e){}
      saveData(); Sound.play('transition');
      Nav.replace('hub'); }

    sec.addEventListener('nav-destroy',()=>{ dead=true; timers.forEach(clearTimeout); });
    build();
    return sec;
  }
};
